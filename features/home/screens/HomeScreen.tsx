      ...
      <Card className="rounded-[17px] border-[#d9d9d9] bg-[#f9f9f9] px-5 py-4">
        <View className="flex-row items-center justify-between gap-4">
          <View className="flex-1 gap-1">
            ...
            <Text
              className="text-[22px] font-semibold text-[#797DEA]"
              variant="body"
            >
...
export const HomeScreen = memo(() => {
  ...
  const { keepReading = [], isFetching } = useLibrarySections();
  const { data: picks = [] } = useNytBestsellers();

  const firstName = user?.name?.split(' ')[0] ?? 'there';
  const initials = user ? getInitials(user) : '?';
  const nowReading = keepReading[0] ?? null;
      ...
      <Container className="gap-6 pb-6">
        ...
        {isFetching ? (
          ...
          <View className="gap-3">
            ...
            <NowReadingCard
              book={nowReading}
              onPress={() => router.push(bookHref(nowReading.id))}
            />
          </View>
        ) : (
          <Card className="items-center gap-3 rounded-[20px] border-[#e8e8e8] bg-[#f9f9f9] py-8">
